...
# to not be overly dependent on an input image, time steps and 
# bands are simulated by replacing the matrix contained in the dummy stars object
...
t2 <- floor(matrix(runif(42, -250, 50), ncol = 7)) # and only for 6x7
...
s <- make_dummy_stars(6, 7, 2.5, -.5714286, -1.14) # mainly used
...
pl_stack = function(s, x, y, add = TRUE, nrM, imgY = 7) {
...
png("exp_resample_space.png", width = 2500, height = 1000, pointsize = 24)
...
pl_stack(s, 0, yoff2, nrM = 3) # input 1
pl_stack(s, off, yoff2, nrM = 2)
pl_stack(s, 2 * off, yoff2, nrM = 1)
...
# mask1
pl(lowres, off, maskHeight, pal = alpha(greys, 0.5), m = matrix(rep("1", 12), ncol = 4), border = 1)
...
# mask2
pl(highres, off + off2, maskHeight, pal = alpha(greys, 0.5), m = matrix(rep(1, 168), ncol = 14), border = 1)
...
pl(s, 7, 0, pal = blues, m = b1)
pl(s, 7, 10, pal = blues, m = b2)
pl(s, 7, 20, pal = blues, m = b3)
pl(s, 14, 0, pal = greens, m = g1)
pl(s, 14, 10, pal = greens, m = g2)
pl(s, 14, 20, pal = greens, m = g3)
pl(s, 21, 0, pal = reds, m = r1)
pl(s, 21, 10, pal = reds, m = r2)
pl(s, 21, 20, pal = reds, m = r3)
pl(s, 28, 0, pal = purples, m = n1)
pl(s, 28, 10, pal = purples, m = n2)
pl(s, 28, 20, pal = purples, m = n3)
...
print_alpha_grid <- function(x,y, alp = 0.2, geom = FALSE) {
  pl(s, 0+x, 0+y, pal = alpha(blues, alp), print_geom = geom,  m = b1, border = 1)
  pl(s, 0+x, 10+y, pal = alpha(blues, alp), print_geom = geom,  m = b2, border = 1)
  pl(s, 0+x, 20+y, pal = alpha(blues, alp), print_geom = geom,  m = b3, border = 1)
  pl(s, 7+x, 0+y, pal = alpha(greens, alp), print_geom = geom,  m = g1, border = 1)
  pl(s, 7+x, 10+y, pal = alpha(greens, alp), print_geom = geom,  m = g2, border = 1)
  pl(s, 7+x, 20+y, pal = alpha(greens, alp), print_geom = geom,  m = g3, border = 1)
  pl(s, 14+x, 0+y, pal = alpha(reds, alp), print_geom = geom,  m = r1, border = 1)
  pl(s, 14+x, 10+y, pal = alpha(reds, alp), print_geom = geom,  m = r2, border = 1)
  pl(s, 14+x, 20+y, pal = alpha(reds, alp), print_geom = geom,  m = r3, border = 1)
  pl(s, 21+x, 0+y, pal = alpha(purples, alp), print_geom = geom,  m = n1, border = 1)
  pl(s, 21+x, 10+y, pal = alpha(purples, alp), print_geom = geom,  m = n2, border = 1)
  pl(s, 21+x, 20+y, pal = alpha(purples, alp), print_geom = geom,  m = n3, border = 1)
}
...
segments(108,-2.4, 112,-3.2, lwd=3)
segments(114,3.2, 118,2.4, lwd=3)
...
print_grid_filter <- function(x, y) {
  pl(s, 0+x, 0+y, pal = blues, m = matrix(b1[mask == TRUE], ncol = 7))
  pl(s, 0+x, 10+y, pal = blues, m = matrix(b2[mask == TRUE], ncol = 7))
  pl(s, 0+x, 20+y, pal = blues, m = matrix(b3[mask == TRUE], ncol = 7))
  pl(s, 7+x, 0+y, pal = greens, m = matrix(g1[mask == TRUE], ncol = 7))
  pl(s, 7+x, 10+y, pal = greens, m = matrix(g2[mask == TRUE], ncol = 7))
  pl(s, 7+x, 20+y, pal = greens, m = matrix(g3[mask == TRUE], ncol = 7))
  pl(s, 14+x, 0+y, pal = reds, m = matrix(r1[mask == TRUE], ncol = 7))
  pl(s, 14+x, 10+y, pal = reds, m = matrix(r2[mask == TRUE], ncol = 7))
  pl(s, 14+x, 20+y, pal = reds, m = matrix(r3[mask == TRUE], ncol = 7))
  pl(s, 21+x, 0+y, pal = purples, m = matrix(n1[mask == TRUE], ncol = 7))
  pl(s, 21+x, 10+y, pal = purples, m = matrix(n2[mask == TRUE], ncol = 7))
  pl(s, 21+x, 20+y, pal = purples, m = matrix(n3[mask == TRUE], ncol = 7))
}
...
draw_arrow <- function(x, y, flag = FALSE) {
  # seg <- matrix(c(c(-0.5,0,1.5,3), c(-0.5,0,1.5,3),
  #                 c(-.9,1,2.5,3.7), c(-.9,1,2.5,3.7)), ncol = 4)
  seg <- matrix(c(c(-1.0, 0.3, 1.5, 2.7, 3.9, 5.1), c(-1.0, 0.3, 1.5, 2.7, 3.9, 5.1),
                  c(-0.5, 0.7, 1.9, 3.1, 4.3, 5.6), c(-0.5, 0.7, 1.9, 3.1, 4.3, 5.6)), ncol = 4)
  if(flag) {
    seg <- matrix(c(c(-1.0, 1.5, 2.7, 3.9), c(-1.0, 1.5, 2.7, 3.9),
  ...
  seg[,2] <- seg[,2] + y
  seg[,4] <- seg[,4] + y
  segments(seg[,1], seg[,2], seg[,3], seg[,4], lwd = 4)
}
...
y = 10 # 7.5
plot.window(xlim = c(0, x), ylim = c(0, y), asp = 1)
pl(s, 4.8, 3.8, pal = blues, m = b3, breaks = brks)
print_text(s, 4.8, 3.8, m = b3)
pl(s, 3.6, 2.6, pal = blues, m = b11, breaks = brks)
print_text(s, 3.6, 2.6, m = b11)
pl(s, 2.4, 1.4, pal = blues, m = b12, breaks = brks)
print_text(s, 2.4, 1.4, m = b12)
pl(s, 1.2, .2, pal = blues, m = b2, breaks = brks)
print_text(s, 1.2, .2, m = b2)
pl(s, 0, -1, pal = blues, m = b1, breaks = brks)
print_text(s, 0, -1, m = b1) # print text on left first stack
pl(s, 24.8, 3.8, pal = alpha(blues, 0.3), m = matrix(rep("NA", 42), ncol = 7))
pl(s, 23.6, 2.6, pal = blues, m = (b12 + b11 + b3) / 3, breaks = brks)
print_text(s, 23.6, 2.6, m = floor((b12 + b11 + b3) / 3))
pl(s, 22.4, 1.4, pal = blues, m = (b2 + b12 + b11) / 3, breaks = brks)
print_text(s, 22.4, 1.4, m = floor((b2 + b12 + b11) / 3))
pl(s, 21.2, .2, pal = blues, m = (b1 + b2 + b12) / 3, breaks = brks)
print_text(s, 21.2, .2, m = floor((b1 + b2 + b12) / 3))
pl(s, 20, -1, pal = alpha(blues, 0.3), m = matrix(rep("NA", 42), ncol = 7))
draw_arrow(5.7, 1.7)
...
draw_arrow(15.4, 1.7) # draw ma explanation
...
draw_arrow(25.7, 1.7, flag = FALSE)
# draw_arrow(25.7, 1.7)
points(0,0, pch=16)
dev.off()
